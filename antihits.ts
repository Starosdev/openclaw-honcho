...
export const DEFAULT_ANTI_HITS: AntiHitsConfig = {
  enabled: true,
  expandTop: 2,
  expandK: 4,
  maxAntiHits: 3,
...
export function selectAntiHits(
  hits: ConclusionLike[],
  neighboursByHitId: Map<string, ConclusionLike[]>,
...
export async function collectAntiHits(
  scope: ConclusionQuerier,
  query: string,
  options: { topK: number; maxDistance: number; config: AntiHitsConfig },
): Promise<AntiHit[]> {
  ...
  const neighboursByHitId = new Map<string, ConclusionLike[]>();
...
export function formatAntiHits(antiHits: AntiHit[]): string {
  if (antiHits.length === 0) {
    return "";
  }
  const lines = antiHits.map(
    (antiHit) =>
      `- [${isoDay(antiHit.conclusion.createdAt)}] ${antiHit.conclusion.content.replaceAll("\n", " ")}\n  (a newer conclusion from ${isoDay(antiHit.supersededBy.createdAt)} covers the same ground)`,
  );
  return [
