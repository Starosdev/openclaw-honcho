...
export async function readLocalEpisodes(config, readFileImpl = readFile) {
    let parsed;
    try {
...
export function formatDegradedContext(episodes) {
...
export async function buildDegradedContext(config, readFileImpl) {
    if (!config.enabled) {
        return "";
    }
